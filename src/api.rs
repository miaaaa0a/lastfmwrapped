...
use crate::{calculations::{calculate_genre_months, calculate_year, largest_value_hashmap}, imageprocessing, lfm, spotify};
...
pub async fn top_5_songs(Path(username): Path<String>) -> Json<Value> {
    ...
    for song in top_tracks_sorted {
        ...
        let song_cover_info = spotify::find_song_cover(&spotify_client, song.0, &top_track_name).await;
        let song_cover_url = song_cover_info["url"].as_str().unwrap_or("").trim_matches('\"');
        let song_cover = reqwest::get(song_cover_url).await.unwrap().bytes().await.unwrap();
        let song_cover_reader = ImageReader::new(Cursor::new(&song_cover)).with_guessed_format().unwrap();
        let song_cover_img = song_cover_reader.decode().unwrap();
...
pub async fn genre_evolution(Path(username): Path<String>) -> Json<Value> {
    ...
    let months = calculate_genre_months(lfm_client, spotify_client).await.unwrap();
...
pub async fn final_image(Path(username): Path<String>) -> Json<Value> {
    ...
    top_tracks_sorted.sort_by_key(|k| k.1);
    top_tracks_sorted.reverse();
    let top_track_names = top_tracks_sorted.iter().map(|(x, _)| x.split(" - ").collect_vec()[1]).collect::<Vec<&str>>();
    ...
    let top_artist_names = top_artists_sorted.iter().map(|x| x.0.as_str()).collect::<Vec<&str>>();

    let icon_info = spotify::find_artist_icon(&spotify_client, &top_artist_names[0].to_string()).await;
    let icon_url = icon_info["url"].as_str().unwrap_or("").trim_matches('\"');
    let icon = reqwest::get(icon_url).await.unwrap().bytes().await.unwrap();
    let icon_reader = ImageReader::new(Cursor::new(&icon)).with_guessed_format().unwrap();
    let icon_img = icon_reader.decode().unwrap();

    let img = imageprocessing::final_image(
        total_minutes, 
        top_track_names, 
        top_artist_names,
        icon_img
